...
function init() {
    ...
    canvas.addEventListener('mousemove', function(e){
        ...
        for (var i = 0; i < cartas.length; i++) {
            if (cartas[i].selec==true) {
                ...
                cartas[i].x = ult_pos_x
                cartas[i].y = ult_pos_y
                draw();
    ...
    cartas=[]
    cards.find().forEach( function(carta_db) {
          carta = new card(widht*carta_db.posicion[0], height*carta_db.posicion[1],TiposCartas[carta_db.name][0], TiposCartas[carta_db.name][1])
          cartas.push(carta);
    } );
