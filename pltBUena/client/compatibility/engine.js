	...
	this.processAccion = function(accion){
		switch(accion.tipo){
			...
			case "accionP":
				...
				that.gameboard.pointsboard.updateTarget(accion.targetName,objeto[1],task);				break;
			case "accionT":
			  		...
			  		if(carta && (!over)){
			 			...
			 			that.gameboard.handboard.updateHand(carta, over, false);
			  			...
			  			n_scroll=parseInt(distancia/90)
							...
							if (n_scroll==1){
								resta=that.gameboard.board.scroll-1
								if (resta>=0){
				  				that.gameboard.board.scroll=resta
				  				init_y_scroll=init_y_scroll+90
								}

			  			}else if (n_scroll==-1){
								suma=that.gameboard.board.scroll+1
								if (suma<=23){
									that.gameboard.board.scroll=suma
				  				init_y_scroll=init_y_scroll-90
								}
			  			}
						}
